...
class Tpl {
	 ...
	 *
	 * @param string|array $setting : name of the setting to configure or associative array type 'setting' => 'value'
	 ...
	 * @param Closure $function : action to do when the tag is parsed
	 */
	public static function registerTag($tag, $parse, $function) {
		static::$registeredTags[$tag] = ["parse" => $parse, "function" => $function];
	}
	...
	protected function checkTemplate($template) {
		...
		if ($this->config['debug'] || !file_exists($parsedTemplateFilepath) || (filemtime($parsedTemplateFilepath) < filemtime($templateFilepath))) {
			$parser = new Tpl\Parser($this->config, static::$plugins, static::$registeredTags);
