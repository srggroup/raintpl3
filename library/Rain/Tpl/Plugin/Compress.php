...
class Compress extends Plugin {
	...
	protected static $configure = [
		'html'       => ['status' => true],
		'css'        => ['status' => true],
		'javascript' => ['status' => true, 'position' => 'bottom'],
	];

	protected $hooks = ['afterDraw'];
	 ...
	 */
	public function afterDraw(ArrayObject $context) {

		// get the cache directory
		$this->cacheDir = $context->conf['cache_dir'];
	...
	protected function compressCSS($html) {
		...
		if (!preg_match_all("/<link.*href=\"(.*?\.css)\".*>/", $html, $matches)) {
			return $html;
		} // return the HTML if doesn't find any
		...
		$cacheFolder = $this->cacheDir . "compress/css/"; // css cache folder
				...
				if (empty($urlArray[$url])) {
					...
					if (preg_match_all("/url\({0,1}(.*?)\)/", $stylesheetFile, $matches)) {
						foreach ($matches[1] as $imageUrl) {
							...
							$real_path = Parser::reducePath("../../../" . dirname($url) . "/" . $imageUrl);
	...
	protected function compressJavascript($html) {
		...
		$cachedFilename = md5($md5Name);
		$cacheFolder = $this->cacheDir . "compress/js/"; // css cache folder
