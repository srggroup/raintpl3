...
//@phpcs:ignoreFile Squiz.NamingConventions.ValidVariableName.NotCamelCaps
...
class ImageResize extends Plugin {
	...
	public function beforeParse(ArrayObject $context) {
		// set variables
		$html = $context->code;
		...
		if (preg_match_all('/<img((?:\s*(src="(?<src>.*?)"))|(\s*(width="(?<width>.*?)"))|(\s*height="(?<height>.*?)")|(\s*resize="(?<resize>.*?)")|(\s*crop="(?<crop>.*?)"))*.*?>/', $html, $matches)) {
			...
			for ($i = 0, $n = count($matches[0]); $i < $n; $i++) {
				...
				if ($auto_crop) {
					$crop = $matches['crop'][$i] != 'false';
				} else {
					$crop = $matches['crop'][$i] == 'true';
				}
	...
	public static function imgResize($src, $dest, $w, $h, $quality, $crop) {
		...
		$dest .= 'img.' . md5($src . $crop . $quality) . $w . 'x' . $h . '.' . $ext;
